...
Draw Debug Lines
----------------
Draws some lines that were included for debugging purposes
Chooses a random starting angle on each run/reset

Center-click to reset the iterations and choose a new angle
...
boolean enableDebugCheckLines = false; // Draws all the intersection checking iterations (slow)

int delayBetweenSteps = 10; // Change how fast it iterates
...
void draw() {
  ...
  path.setMaxIterations(maxIter); // Set the max iterations
  ...
  Sketch.drawDebugVectors(); // Draws the normals for all the bounces from the start point
  if(enableDebugCheckLines) {
    Sketch.drawCheckLines(); // Draws all the intersection checking iterations (slow)
  }
  ...
  maxIter++; // Increase the iteration number
  delay(delayBetweenSteps); // Wait for a hot sec
}
